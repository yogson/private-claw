...
Callback signing and verification for model selection UI.
"""
...
def sign_model_callback(model_id: str, chat_id: int, secret: bytes) -> str:
    """Build compact signed callback payload for model selection buttons."""
    ts = int(datetime.now(UTC).timestamp())
    ts36 = format(ts, "x")
    msg = f"{chat_id}:{model_id}:{ts36}"
    sig = hmac.new(secret, msg.encode(), hashlib.sha256).hexdigest()[:_COMPACT_HMAC_SIG_LENGTH]
    return f"{COMPACT_CALLBACK_ACTION}:{model_id}:{ts36}:{sig}"


def verify_model_callback(callback_data: str, expected_chat_id: int, secret: bytes) -> str | None:
    """Return verified model_id for compact callback payloads."""
    ...
    if not without_ts or not ts36:
        return None
    action, _, model_id = without_ts.partition(":")
    if action != COMPACT_CALLBACK_ACTION or not model_id:
        return None
    try:
        ts = int(ts36, 16)
    except ValueError:
    ...
    if age < 0 or age > CALLBACK_TTL_SECONDS:
        return None
    msg = f"{expected_chat_id}:{model_id}:{ts36}"
